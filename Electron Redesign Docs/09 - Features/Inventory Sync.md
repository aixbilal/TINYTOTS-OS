...
Inventory synchronization between the POS and backend.

## Scope

This document defines the operational capability, workflow, business behavior, system boundaries, and implementation considerations for **Inventory Sync**.
...
Online, offline, retry, failure, and conflict behavior must be grounded in the redesign specification and verified implementation. Conflict resolution must not be invented if the sources do not define it.
