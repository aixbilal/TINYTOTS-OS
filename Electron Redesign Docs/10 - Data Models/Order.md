...
Transaction/order aggregate and lifecycle.
...
`Order` represents the transaction-level aggregate. Individual products/variants belong to **Order Item** records; payment and receipt records remain separate data concerns.
...
The exact lifecycle and status model must be source-supported.
