...
// Elegant serif for the large hero heading only — everything else stays in
// the brand's sans-serif for readability, per the request to keep the rest
// of the UI in a modern sans-serif.
...
type SocialLink = { id: number; platform: "instagram" | "facebook" | "pinterest" | "tiktok"; account_name: string; url: string };
  ...
  return (
    <div className="grid grid-cols-12 items-center gap-[1.5vw] px-[2.5vw] py-[2vh] flex-1 min-h-0">
      ...
      <div className="col-span-3 flex flex-col gap-3">
        ...
        <h1
          className={`${playfair.className} text-[clamp(2.5rem,4.2vw,4.5rem)] leading-[0.95] font-bold text-on-surface`}
        >
      ...
      {campaign.hero_mode === "single_image" && campaign.hero_banner_image ? (
        /* Mode 1: one pre-composed banner image (kid + product + lifestyle
           already baked in by the admin) spans the center + lifestyle
           columns. Text and statistics stay code-rendered on top/beside so
           they remain editable without recreating the whole banner. */
        <div className="col-span-7 relative h-full flex items-center">
          <div className="relative w-full h-[92%] rounded-3xl overflow-hidden shadow-xl">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={campaign.hero_banner_image} alt="" className="w-full h-full object-cover" />
            {campaign.hero_badge && (
              <div className="absolute top-6 right-8 w-[14%] aspect-square rounded-full bg-surface border border-outline-variant/30 shadow-lg flex items-center justify-center text-center">
        ...
        <>
          {/* Mode 2: Center: floating hero product + feature list */}
    ...
    <div className="bg-[#3b241a] text-white flex items-center justify-between px-[2.5vw] py-[1.6vh] shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        {socialLinks.map((s) => (
          ...
          >
            <span className="material-symbols-outlined text-[16px]">{PLATFORM_ICON[s.platform] || "link"}</span>
          </a>
        ))}
      </div>
      <div className="flex items-center gap-3">
